...
# SDD、TDD 出發點都是好的，但你懶得出發

很多開發方法都會因為人的懶惰而困難重重，AI 至少能讓你寫出點東西，但不能根本解決懶惰

SDD 

1. 規格自己懶得寫，寫不出個所以然
2. 有些問題只有在實作過程中才會發覺的
3. AI 寫會生一堆文件你看不完，而且你其實也懶得看
4. AI 照著做完了，真的有嗎，你真的會看嗎？
5. 改規格文件的速度跟不上改 Code 的速度，甚至直接被遺忘
...
優先以人類覺得最正確的邏輯對型別進行建模，無法精確表達通常是型別~~玩不夠花~~，或是已經觸碰到型別系統的限制了。

<v-click>
這方面 AI 其實玩的比你還溜，叫他設計看看，你順便偷學一下就好了
</v-click>
...
Brand Type 區隔不該被相容的型別，Conditional Type 從既有型別推導出下一個型別。
...
<v-switch at="1">
  <template #0>
    資料在 boundary 被驗證後，Brand Type 讓它在型別系統中留下「可信身份」。
  </template>
  <template #1>
    Conditional Type 可以描述輸入型別與輸出型別的關係，常用來從既有型別抽出資訊。
  </template>
  <template #2>
    Template Literal Type 搭配 Conditional Type，可以從 route 字串中抽出參數名稱，再組成對應的 params 型別。
  </template>
</v-switch>
...
- 型別不只是補完與提示，而是程式設計重要的一環
- 更深入地理解型別系統，能讓我們把規則放到更適合的位置
- 能靜態保證的規則，交給型別系統；無法靜態保證的規則，集中在 boundary 或 runtime 處理
- 好的型別設計，會輔佐你完成所有驗證與處理流程

身為程式的**設計**者，不只是在「寫出能跑的程式」，而是有意識地設計一套讓程式不容易被誤用的結構，讓錯誤更早暴露出來。

在一個思考如何實作不再是問題的時代，型別的規劃與設計能力顯得更為重要，因為它同時也是 AI 在實作時一個強而有力的約束。
...
開始意識到型別的重要性之後，我獲得了些什麼技能

- 你可以利用編譯器幫你盤點所有使用該函數/元件的地方
- 可以光讀型別就知道大概怎麼串接，甚至不用看文件
- 可以刻意的去戳 lib 一些型別允許但實際不合理的組合，藉此推敲邊界條件，避免自己採坑
- 適用所有靜態型別語言，理解基礎語法後，熟悉型別系統後通常能快速適應語言生態
